    ...
    where step (pos, ld) (AsmLine lbls insn) =
          ...
          advance (Just (Statement (WithPos _ nmemonic) opr)) pos =
              case nmemonic of
                ...
                ".align" -> align (toWordAddr pos) opr
                _ -> wordAdvance pos 1
...
generate pos ld lns =
  ...
  where
    ...
    concatBytes [] = []
    concatBytes [Pad _] = []  -- ignore trailing paddings
    concatBytes (Pad n:xs) = replicate n (0::Word16) : concatBytes xs
...
generateOne ld pos statement@(Statement (WithPos nmemonicOffset nmemonic) opr) =
  -- directives that generate other than one-word code
    case nmemonic of
      ...
      ".dc.b" -> let a = toByteAddr pos
                 in
                   if null opr then missingOperands
                   else do
      ...
      ".align" -> do
          _ <- needOneArgument
          let addr = toWordAddr pos
          align <- getData Nothing False 2 addr (head opr)
          -- must be a power of 2
          if align <= 0 || align > 0x8000 || popCount (toWord16 align) /= 1
              then genErr (findOffset (head opr)) "bad argument to .align"
              else
                let newa = alignAddr align addr
                in
                  return $ (newa ,Pad (fromIntegral (getAddr newa) - fromIntegral (getAddr addr)))
  ...
  where
    genErr = generatorError (pos, Words [])
    errorMsg = "missing arguments for " ++ nmemonic
    missingOperands = genErr nmemonicOffset errorMsg
    needOneArgument = case length opr of
      0 -> missingOperands
      1 -> return (pos, Words [])
      _ -> genErr (findOffset (head (tail opr)))
                  ("too many arguments to " ++ nmemonic)

    getData lbl addressAllowed badVal addr expr = do
        val <- calcExpr lbl addr expr
        case (isAbsolute pos, addressAllowed, val) of
          (_, _, Bad) -> return badVal -- errors has registered.
          (_, _, Num n) -> return n
          (True, True, Address a) -> return $ fromIntegral a
          (_, False, _) -> generatorError badVal (findOffset expr) "address not allowed"
          (False, _, _) -> generatorError badVal (findOffset expr) "relocatable address not allowed"

    toWord8 v = (fromIntegral v) :: Word8
    getByte a e = getData (Just ld) False 0 a e >>= pure . toWord8 >>= checkValueRange 0 0xff e
    getWord a e = getData (Just ld) True 0 a e >>= pure . toWord16 >>= checkValueRange 0 0xffff e
    toWord32 v = (fromIntegral v) :: Word32
    getLong a e = getData (Just ld) True 0 a e >>= pure . toWord32
...
generateWord ld pos statement@(Statement (WithPos nmemonicOffset nmemonic) _) = generateOne' statement
  where
    ...
    genBranch pat opr = do
        target <- calcExpr (Just ld) pos opr
        case target of
...
genJump ld pos pat (Statement _ [opr]) = (jump =<< (calcExpr (Just ld) pos opr))
  where
